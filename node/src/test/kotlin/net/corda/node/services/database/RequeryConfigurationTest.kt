...
//            requerySession.insert(createVaultStateEntity(txn))
    ...
    private fun createVaultStateEntity(txn: SignedTransaction): VaultStatesEntity {
        ...
        state.txId = txnState.txhash.toString()
        state.index = txnState.index
        state.stateStatus = Vault.StateStatus.UNCONSUMED
        state.contractStateClassName = DummyContract.SingleOwnerState::class.java.name
        state.contractState = DummyContract.SingleOwnerState(owner = DUMMY_PUBKEY_1).serialize(createKryo()).bytes
        state.notaryName = txn.tx.notary!!.name
        state.notaryKey = txn.tx.notary!!.owningKey.toBase58String()
        state.recordedTime = Instant.now()
        return state
    }
